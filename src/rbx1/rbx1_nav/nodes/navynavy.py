    ...
    def doSomething(self, msg):
        try:
            ...
            if not self.is_visible:
                ...
                rospy.loginfo(marker.pose.pose.position.x)
                xt = marker.pose.pose.position.x
                yt = marker.pose.pose.position.y
                ...
                rospy.loginfo("position @ (%f, %f)", xt, yt)
                rospy.loginfo("yaw, pitch, roll @ (%f, %f, %f)", yaw, pitch, roll)

                # goal = MoveBaseGoal()
                # goal.target_pose.header.frame_id = 'map'
                # goal.target_pose.header.stamp = rospy.Time.now()
                if (yaw > 80 and yaw < 100) and (roll > -2 and roll < 5):
                    rospy.loginfo("move 90 x")
                    dx = +0.5; dy = 0; q = Quaternion(0,0,0.7071,0.7071)
                elif (yaw > 80 and yaw < 100) and (roll > -81 and roll < -100):
                    rospy.loginfo("move -90 x")
                    dx = 0; dy = +0.5; q = Quaternion(0,0,0.7071,0.7071)
                elif (yaw > -80 and yaw < -100) and (roll > -5 and roll < 5):
                    rospy.loginfo("move 90 y")
                    dx = -0.5; dy = 0; q = Quaternion(0,0,0.7071,0.7071)
                elif (yaw > 80 and yaw < 100) and (roll > 80 and roll < 100):
                    rospy.loginfo("move -90 y")
                    dx = 0; dy = -0.5; q = Quaternion(0,0,0.7071,0.7071)
                goal.target_pose.pose = Pose(Point(xt +dx, yt +dy, 0), q)
                if marker.id == 1:
                    """
